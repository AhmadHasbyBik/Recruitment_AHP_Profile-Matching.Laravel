        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title">Edit Interview Schedule</h3>
                </div>
                <div class="card-body">
                    <form action="{{ route('interviews.update', $interview->id) }}" method="POST">
                        ...
                        <div class="form-group">
                            ...
                            <select name="feedback_by" id="feedback_by" class="form-control @error('feedback_by') is-invalid @enderror" required>
                                ...
                                @foreach($interviewers as $interviewer)
                                    <option value="{{ $interviewer->id }}" {{ $interview->feedback_by == $interviewer->id ? 'selected' : '' }}>
                                        {{ $interviewer->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <input type="datetime-local" name="schedule_date" id="schedule_date" 
                                   ...
                                   value="{{ $interview->schedule_date->format('Y-m-d\TH:i') }}" required min="{{ now()->format('Y-m-d\TH:i') }}">
                            @error('schedule_date')
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
                        ...
                        <div class="form-group">
                            ...
                            <textarea name="notes" id="notes" rows="3" 
                                      class="form-control @error('notes') is-invalid @enderror">{{ $interview->notes }}</textarea>
                            @error('notes')
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
